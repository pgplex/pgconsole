...
```bash
mkdir pgconsole-worker && cd pgconsole-worker
npm init -y
...
routes = [
  { pattern = "pgconsole.example.com", custom_domain = true }
]

[[containers]]
class_name = "pgconsole"
image = "pgplex/pgconsole:<version>"
max_instances = 1
...
[[migrations]]
tag = "v1"
new_sqlite_classes = ["pgconsole"]
```

Replace `pgconsole.example.com` with your domain. To connect to your own database, mount a config file in the container or use the demo mode image as-is.
...
Your pgconsole instance is now live at your configured domain.
